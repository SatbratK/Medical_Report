...
<body>
    ...
    <form>
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-10">
                <input type="text" class="form-control" id="patient_contact" name="patient_contact" maxlength="10">
            </div>
        </div>
        <div class="form-group row">
...
<script>
    ...
    $('#save_report').click(function () {
        var reportData = {patient_hb: $('#hb_result').val(),

            
        };

        $.ajax({
            url: '{{ route('save_patient_report') }}', // Ensure this route is correct
            type: 'POST',
            data: reportData,
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}' // Include CSRF token
